...
Node.js로 작성한 서버가 초당 수만 건의 요청을 처리할 수 있는 이유는 무엇일까? 싱글 스레드임에도 불구하고 어떻게 그토록 빠른 성능을 낼 수 있는 것일까? 높은 동시성의 핵심은 libuv의 비동기 I/O와 이벤트 루프에 있지만, 그 기반에서 자바스크립트 코드를 빠르게 실행하는 것은 V8 엔진의 역할이다. V8은 구글이 개발한 고성능 자바스크립트 엔진으로, Node.js의 심장부 역할을 담당한다. 자바스크립트 코드를 기계가 이해할 수 있는 형태로 변환하고, 실행 중에 지속적으로 최적화를 수행하여 놀라운 속도를 이끌어낸다.
...
다음은 위의 `add` 함수가 어떤 형태의 바이트코드로 변환되는지 보여주는 예시다(`examples/part1/1-1/1-1-1-bytecode-example.js`).

```bash
node --print-bytecode --print-bytecode-filter=add examples/part1/1-1/1-1-1-bytecode-example.js
```
...
Node.js에서 디옵티마이제이션 발생 여부와 이유를 확인하려면 V8 플래그를 사용할 수 있다. 다음 예제 코드(`examples/part1/1-1/1-1-3-deopt-example.js`)로 실제 동작을 확인해보자.
...
```bash
node --trace-opt --trace-deopt examples/part1/1-1/1-1-3-deopt-example.js
```
...
IC 상태에 따른 최적화 동작을 직접 확인해보자. `--trace-opt`와 `--trace-deopt` 플래그를 사용하면 V8의 최적화 및 디옵티마이제이션 과정을 추적할 수 있다(`examples/part1/1-1/1-1-5-ic-states.js`).

```bash
node --trace-opt --trace-deopt examples/part1/1-1/1-1-5-ic-states.js 2>&1
```

```
[marking ... <JSFunction getXMono ...> for optimization to TURBOFAN, reason: hot and stable]
[completed optimizing ... <JSFunction getXMono ...> (target TURBOFAN)]
--- Monomorphic 테스트 완료 (getXMono 최적화됨) ---
[marking ... <JSFunction getXPoly ...> for optimization to TURBOFAN, reason: hot and stable]
[completed optimizing ... <JSFunction getXPoly ...> (target TURBOFAN)]
--- Polymorphic 테스트 완료 (getXPoly도 최적화되지만 덜 효율적) ---
...
출력에서 핵심적인 부분을 살펴보면, `getXMono`와 `getXPoly` 함수 모두 TurboFan에 의해 최적화된다. 하지만 `add` 함수의 경우, 숫자로만 호출되어 최적화된 후 갑자기 문자열이 전달되자 `bailout (kind: deopt-eager, reason: not a Smi)`라는 디옵티마이제이션이 발생했다. "not a Smi"는 "Small Integer가 아님"을 의미하며, V8이 숫자 덧셈으로 최적화한 가정이 깨졌음을 나타낸다.
...
```bash
node --allow-natives-syntax examples/part1/1-1/1-1-4-ic-invalidation.js
```
...
const arr2 = []
for (let i = 0; i < 100; i++) {
  arr2.push(i) // PACKED 배열 유지
}

// 미리 크기를 할당하고 순차적으로 채우는 것도 가능
const arr3 = new Array(100)
for (let i = 0; i < 100; i++) {
  arr3[i] = i // 순차적 할당으로 PACKED로 전환
}
```

`new Array(100)`은 길이가 100인 배열을 생성하지만, 초기에는 모든 인덱스가 구멍인 HOLEY 배열이 된다. 빈 배열에서 시작해 `push`로 요소를 채우면 처음부터 PACKED 배열이 유지된다. 한편, 미리 크기를 할당한 배열도 for 문으로 인덱스 0부터 순차적으로 채우면 V8이 이를 최적화하여 PACKED 배열처럼 다룬다. 다만 인덱스를 건너뛰거나 무작위 순서로 할당하면 여전히 HOLEY로 남으므로 주의해야 한다.
...
여기서 한 가지 더 깊이 이해해야 할 점이 있다. `fs.readFile`이나 `setTimeout`과 같은 비동기 작업은 libuv가 관리하는 태스크 큐(Macrotask Queue)를 사용하지만, **Promise** 와 `process.nextTick`은 더 높은 우선순위를 가진 별도의 큐를 사용한다. 단, 이 두 가지는 서로 다른 주체가 관리한다. **Promise의 콜백(.then, .catch 등)은 V8이 직접 관리하는 마이크로태스크 큐** 를 사용하는 반면, **`process.nextTick`은 Node.js가 관리하는 별도의 nextTickQueue** 를 사용한다. Node.js 이벤트 루프는 각 단계 사이에서 먼저 nextTickQueue를 비우고, 그 다음에 V8의 마이크로태스크 큐를 비우는 순서로 동작한다. 두 큐 모두 일반 태스크보다 우선순위가 높아, 현재 실행 중인 코드가 끝나자마자(콜 스택이 비워지자마자) 즉시 실행된다.
