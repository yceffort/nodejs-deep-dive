...
파서는 자바스크립트 소스 코드를 읽어 **AST(Abstract Syntax Tree, 추상 구문 트리)** 로 변환한다. AST는 코드의 구조를 트리 형태로 표현한 것으로, 이후 모든 처리의 기반이 된다.

#### 1-1-1-3-2. Ignition(인터프리터)

Ignition은 AST를 **바이트코드(Bytecode)** 로 컴파일하고 이를 직접 실행하는 인터프리터다. 실행 중에 각 연산에서 어떤 타입의 값이 사용되었는지를 **피드백 벡터(Feedback Vector)** 에 기록하며, 이 정보는 이후 JIT 컴파일러들이 최적화 결정을 내리는 핵심 근거가 된다.

#### 1-1-1-3-3. JIT 컴파일러: Sparkplug, Maglev, TurboFan

JIT(Just-In-Time) 컴파일러는 바이트코드를 기계어로 변환하여 실행 속도를 높인다. V8은 세 단계의 JIT 컴파일러를 갖추고 있다.

- **Sparkplug**: 바이트코드를 빠르게 기계어로 변환하는 기본 컴파일러
- **Maglev**: 타입 피드백을 활용한 중간 최적화 컴파일러
- **TurboFan**: 인라이닝, 이스케이프 분석 등 모든 최적화 기법을 동원하는 최종 컴파일러

각 구성 요소의 상세한 동작 원리는 다음 절에서 컴파일 파이프라인을 따라가며 살펴본다.
...
AST Explorer[^12]를 사용하면 실제 AST 구조를 직접 확인할 수 있다. 예를 들어 `const x = 1 + 2`라는 간단한 코드를 acorn 파서로 파싱하면 다음과 같은 JSON 구조가 생성된다.
...
V8은 파싱 비용을 줄이기 위해 **지연 파싱(Lazy Parsing)** 기법을 사용한다[^13]. 모든 함수를 처음부터 완전히 파싱하지 않고, 함수 선언만 기록해두었다가 실제로 호출될 때 완전한 파싱을 수행한다. 예를 들어 다음과 같은 코드가 있다고 하자.
...
Ignition은 바이트코드를 하나씩 읽어서(fetch) 해석하고(decode) 실행하는(execute) 과정을 반복한다. 각 바이트코드 명령어에는 그에 대응하는 실행 코드(핸들러)가 있어서, 인터프리터는 명령어를 읽을 때마다 해당 핸들러를 찾아 실행한다. 이 핸들러들 자체가 기계어로 최적화되어 있어 인터프리터치고는 상당히 빠르지만, 그래도 한계는 있다[^4].
...
> **V8 내부 구현 살펴보기**
>
> Ignition의 실행 흐름을 좀 더 자세히 들여다보면, 함수가 호출될 때 **InterpreterEntryTrampoline**이라는 진입점 코드가 먼저 실행된다. 이 코드는 함수 실행에 필요한 스택 프레임(지역 변수, 반환 주소 등을 저장하는 메모리 영역)을 설정한 뒤, 첫 번째 바이트코드의 핸들러로 점프한다.
>
> 각 핸들러는 자신의 작업을 마친 후 **디스패치 테이블**(바이트코드 종류별 핸들러 주소를 담은 배열)을 참조하여 다음 바이트코드의 핸들러로 직접 점프한다. 일반적인 함수 호출이 아니라 **꼬리 호출(tail call)** 방식으로 점프하기 때문에 스택이 계속 쌓이지 않는다. 이러한 방식을 **간접 스레디드 인터프리터(indirectly threaded interpreter)**라고 부른다. "간접"이라는 이름은 바이트코드가 핸들러 주소를 직접 담고 있지 않고, 테이블을 통해 간접적으로 찾기 때문에 붙었다.
...
TurboFan은 바이트코드를 바로 기계어로 변환하지 않고, 먼저 **중간 표현(IR, Intermediate Representation)** 이라는 형태로 변환한다[^5]. 중간 표현은 코드의 의미를 분석하고 최적화하기 쉬운 형태로 표현한 것이다. TurboFan은 이 중간 표현 위에서 다양한 최적화를 수행한 뒤, 최종적으로 기계어 코드를 생성한다.

> **심화: Sea of Nodes**
>
> TurboFan이 사용하는 중간 표현은 **Sea of Nodes**라는 구조다. 전통적인 컴파일러는 "코드가 어떤 순서로 실행되는지"(제어 흐름)와 "데이터가 어떻게 흘러가는지"(데이터 흐름)를 별도로 관리한다. 반면 Sea of Nodes는 이 둘을 하나의 그래프로 통합한다. 이 설계 덕분에 명령어를 자유롭게 재배치하거나 중복 계산을 제거하는 등의 최적화가 더 효과적으로 이루어진다.
...
Maglev도 TurboFan처럼 **추측적 최적화**를 지원한다. 피드백 벡터의 타입 정보를 활용하여 최적화된 코드를 생성하고, 타입 가정이 깨지면 디옵티마이제이션을 수행한다.

> **심화: Maglev의 내부 구조**
>
> Maglev의 컴파일 과정을 좀 더 자세히 살펴보면 다음과 같다. 먼저 **사전 패스(prepass)** 에서 바이트코드를 분석하여 분기와 루프 변수를 파악한다. 그다음 **그래프 빌딩** 단계에서 바이트코드를 **SSA(Static Single Assignment)** 노드로 변환한다. SSA란 각 변수가 딱 한 번만 값을 할당받도록 코드를 변환하는 기법으로, 최적화 분석을 단순하게 만들어준다. 예를 들어 `x = 1; x = 2;`는 SSA에서 `x1 = 1; x2 = 2;`처럼 서로 다른 변수로 취급된다.
>
> if문처럼 여러 경로에서 같은 변수에 값이 할당될 수 있는 경우에는 **Phi 노드**를 사용한다. Phi 노드는 "어느 경로로 왔느냐에 따라 이 값을 사용하라"는 의미다. 마지막으로 **레지스터 할당기**가 각 값을 CPU 레지스터에 배치한다[^7].
