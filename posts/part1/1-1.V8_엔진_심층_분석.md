...
하지만 2000년대 중반에 접어들면서 상황이 급변했다. Gmail(2004년), Google Maps(2005년)와 같은 웹 애플리케이션이 등장하면서 자바스크립트의 역할이 완전히 달라졌다. 이제 자바스크립트는 수천, 수만 줄의 복잡한 로직을 처리해야 했고, 기존 엔진들의 성능은 턱없이 부족했다. Google Maps에서 지도를 드래그할 때마다 버벅거리는 경험은 당시 개발자들에게 익숙한 것이었다.

2008년 9월, 구글은 크롬 브라우저와 함께 V8 엔진을 공개했다. V8은 자바스크립트 엔진의 패러다임을 완전히 바꿔놓았다. 기존 엔진들이 인터프리터 방식에 머물러 있을 때, V8은 JIT(Just-In-Time) 컴파일이라는 혁신적인 접근 방식을 도입했다. 자바스크립트 코드를 실행 시점에 기계어로 직접 컴파일하여, 인터프리터의 오버헤드를 제거한 것이다.

### 1-1-1-2. 동적 타입 언어의 딜레마

V8 팀의 목표는 "the fastest JavaScript runtime on the planet"(세계에서 가장 빠른 자바스크립트 런타임)을 만드는 것이었다[^1]. 동적 타입의 느슨한 언어에서 이러한 목표를 달성하는 것은 대담한 도전이었다. 이것이 왜 어려운 목표인지 이해하려면 자바스크립트의 본질적인 특성을 알아야 한다. 자바스크립트는 동적 타입 언어다. 변수의 타입이 실행 중에 바뀔 수 있고, 객체에 언제든 새로운 속성을 추가하거나 삭제할 수 있다.
...
이러한 유연성은 개발자에게 편리하지만, 엔진 입장에서는 최적화를 극도로 어렵게 만드는 요소다. C++이나 Java 같은 정적 타입 언어에서는 컴파일 시점에 모든 변수의 타입과 객체의 구조가 확정된다. 컴파일러는 이 정보를 활용하여 매우 효율적인 기계어 코드를 생성할 수 있다. 반면 자바스크립트 엔진은 `value`가 숫자인지, 문자열인지, 객체인지 실행해보기 전까지 알 수 없다. 매번 타입을 확인하고 적절한 연산을 선택해야 한다면 성능 저하는 불가피하다.

그렇다면 V8은 이 문제를 어떻게 해결했을까? 핵심 아이디어는 의외로 단순하다. **"미리 알 수 없다면, 실행하면서 배우자."** 이 전략을 구현하기 위해 V8은 여러 구성 요소로 이루어진 정교한 시스템을 갖추고 있다.

### 1-1-1-3. Node.js와 V8 엔진의 구성 요소

V8의 내부 구조를 이해하기 전에, 먼저 Node.js 전체에서 V8이 어떤 위치를 차지하는지 살펴보자.
...
V8 내부는 파서와 여러 단계의 컴파일러로 구성된다. 각 구성 요소의 역할을 살펴보자.

#### 1-1-1-3-1. Parser(파서)

파서는 자바스크립트 소스 코드를 읽어 **AST(Abstract Syntax Tree, 추상 구문 트리)** 로 변환한다. AST는 코드의 구조를 트리 형태로 표현한 것으로, 이후 모든 처리의 기반이 된다. AST Explorer[^12]를 사용하면 실제 AST 구조를 직접 확인할 수 있다. 예를 들어 `const x = 1 + 2`라는 간단한 코드를 acorn 파서로 파싱하면 다음과 같은 트리 구조가 생성된다.
...
AST 구조를 살펴보면 프로그램(`Program`) 안에 변수 선언(`VariableDeclaration`)이 있고, 그 안에 선언자(`VariableDeclarator`)가 있다. 선언자는 식별자 `x`와 초기화 값(`init`)을 포함하며, 초기화 값은 이항 연산(`BinaryExpression`)으로 표현된다. `start`와 `end`는 해당 노드가 소스 코드에서 차지하는 위치를 나타낸다. V8은 이 트리를 순회하면서 바이트코드를 생성한다.

V8의 파서는 **지연 파싱(Lazy Parsing)** 이라는 최적화 기법을 사용한다[^13]. 모든 코드를 즉시 파싱하는 대신, 함수 본문은 실제로 호출될 때까지 완전한 파싱을 미룬다. 처음 코드를 로드할 때 V8은 **프리파서(preparser)** 로 함수의 문법 오류만 빠르게 확인하고 넘어간다. 이후 해당 함수가 실제로 호출되면 그제서야 완전한 AST를 생성하고 바이트코드로 컴파일한다. 브라우저에서 로드되는 자바스크립트나 Node.js 애플리케이션에서 `require`로 불러오는 모듈 중 상당수는 실제로 실행되지 않는 경우가 많기 때문에, 지연 파싱은 초기 로딩 시간을 크게 단축한다.

#### 1-1-1-3-2. Ignition(인터프리터)

Ignition은 AST를 **바이트코드(Bytecode)** 로 컴파일하고 이를 직접 실행하는 인터프리터다. 바이트코드는 기계어보다 추상화된 중간 형태로, CPU 아키텍처에 독립적이면서도 AST보다 훨씬 컴팩트하다. V8 공식 블로그에 따르면 바이트코드는 기존 기계어 코드 대비 25~50% 수준의 크기를 가진다[^3].

Ignition의 핵심 역할은 단순한 코드 실행을 넘어선다. 실행 중에 각 연산에서 어떤 타입의 값이 사용되었는지를 **피드백 벡터(Feedback Vector)** 에 기록한다. "이 덧셈 연산에서는 항상 정수가 사용되었다", "이 속성 접근에서는 항상 같은 형태의 객체가 사용되었다" 같은 정보를 수집하는 것이다. 이 정보는 이후 JIT 컴파일러들이 최적화 결정을 내리는 핵심 근거가 된다. 피드백 벡터의 구체적인 동작 방식은 뒤에서 자세히 다룬다.

#### 1-1-1-3-3. JIT 컴파일러: Sparkplug, Maglev, TurboFan

JIT(Just-In-Time) 컴파일러는 바이트코드를 기계어로 변환하여 실행 속도를 높인다. V8은 세 단계의 JIT 컴파일러를 갖추고 있으며, 각각 컴파일 속도와 생성 코드 품질 사이에서 다른 균형점을 선택한다.

**Sparkplug** 는 2021년에 도입된 기본 JIT 컴파일러다. 바이트코드를 거의 1:1로 기계어로 변환하며, 복잡한 최적화를 수행하지 않는다. 컴파일 속도가 매우 빨라서 Ignition이 바이트코드를 생성하는 즉시 기계어로 변환할 수 있다. Sparkplug가 생성한 코드는 Ignition 인터프리터보다 빠르지만, 최적화된 코드와 비교하면 여전히 느리다. Sparkplug의 역할은 "빠른 시작"과 "본격적인 최적화" 사이의 간극을 메우는 것이다.

**Maglev** 는 2023년 Chrome M117에서 도입된 중간 최적화 컴파일러다. Ignition이 수집한 타입 피드백을 활용하여 타입에 특화된 코드를 생성한다. 예를 들어 "이 덧셈은 항상 정수"라는 피드백이 있다면, 타입 체크를 생략하고 정수 덧셈 명령어만 생성한다. 다만 TurboFan처럼 인라이닝이나 이스케이프 분석 같은 공격적인 최적화는 수행하지 않는다. 컴파일 속도는 TurboFan의 약 10배, 생성 코드 품질은 Sparkplug보다 훨씬 우수하다[^7].

**TurboFan** 은 V8의 최종 최적화 컴파일러다. **Sea of Nodes** 라는 고급 중간 표현(IR)을 사용하여 인라이닝, 이스케이프 분석, 루프 최적화, 타입 특화 등 모든 최적화 기법을 총동원한다. 컴파일에 상당한 시간이 걸리지만, 생성되는 코드는 최고 수준의 성능을 보여준다. TurboFan은 아주 자주 실행되는 "핫(hot)" 함수에만 적용되며, 컴파일 비용은 빠른 실행 속도로 충분히 상쇄된다.

#### 1-1-1-3-4. 다단계 구조의 필요성

이 다단계 구조가 필요한 이유는 **컴파일 시간과 실행 속도 사이의 트레이드오프** 때문이다. 처음부터 TurboFan으로 모든 코드를 컴파일하면 시작 시간이 너무 길어진다. 반대로 Ignition만 사용하면 실행 속도가 느리다.

| 단계 | 컴파일러 | 컴파일 속도 | 실행 속도 | 적용 시점 |
|------|----------|-------------|-----------|-----------|
| 1 | Ignition | 매우 빠름 | 느림 | 모든 코드 |
| 2 | Sparkplug | 빠름 | 보통 | 모든 코드 |
| 3 | Maglev | 보통 | 빠름 | ~400회 호출 |
| 4 | TurboFan | 느림 | 매우 빠름 | ~3000회 호출 |

> 적용 시점의 호출 횟수는 V8 소스 코드의 기본값 기준이며, 플랫폼이나 설정에 따라 달라질 수 있다[^11].

V8은 코드를 먼저 빠르게 실행하고, 자주 사용되는 부분만 선택적으로 더 강력하게 최적화하는 전략을 취한다. 대부분의 코드는 Sparkplug 수준에서 실행되고, 정말 자주 호출되는 핵심 함수만 TurboFan의 최적화 혜택을 받는다.

### 1-1-1-4. V8의 해법: 추측하고 검증하기

V8의 최적화 전략을 이해하기 위해 다음 함수를 생각해보자.
...
앞서 살펴본 것처럼, 함수가 약 400번 호출되면 Maglev가, 약 3000번 호출되면 TurboFan이 동작한다[^11]. 이 가정이 맞는 한, TurboFan은 타입 체크 없이 정수 덧셈만 수행하는 최적화된 기계어 코드를 생성할 수 있다. 만약 가정이 틀리면(갑자기 문자열이 전달되면)? 그때는 최적화된 코드를 버리고 다시 일반적인 코드로 돌아가면 된다. 이것이 바로 V8의 **추측적 최적화(Speculative Optimization)** 다[^2].
...
이 전략이 효과적인 이유는 **실제 코드에서 타입이 급격히 바뀌는 경우가 드물기 때문** 이다. `add(a, b)` 함수가 수천 번 정수로 호출되다가 갑자기 문자열로 호출되는 경우는 현실에서 거의 없다. V8은 이 경험적 사실을 활용한다. 추측이 맞을 때의 이득이 틀릴 때의 비용보다 훨씬 크기 때문에, 전체적으로는 큰 성능 향상을 얻을 수 있다.
...
### 1-1-1-5. 타입 피드백: V8이 타입을 기억하는 방법

V8이 "이 함수는 정수만 받았다"는 것을 어떻게 알 수 있을까? 이 역할을 하는 것이 **타입 피드백(Type Feedback)** 시스템이다.

타입 피드백이란 코드가 실제로 실행되는 동안 각 연산에서 어떤 타입의 값이 사용되었는지를 기록하는 메커니즘이다. V8의 인터프리터(Ignition)가 코드를 실행하는 동안, 각 연산에 어떤 타입의 값이 전달되었는지를 **피드백 벡터(Feedback Vector)** 라는 자료구조에 기록한다[^2].

피드백 벡터를 일종의 "관찰 일지"라고 생각하면 된다. 함수 내의 각 연산마다 슬롯이 하나씩 할당되고, 해당 연산이 실행될 때마다 관찰된 타입 정보가 기록된다.
...
예를 들어 슬롯 0에 해당하는 `x + y` 연산이 실행될 때마다, 피연산자가 정수였는지, 부동소수점이었는지, 문자열이었는지가 누적된다. 이 피드백 정보는 나중에 최적화 컴파일러(TurboFan)가 최적화 결정을 내리는 근거가 된다.

타입 피드백은 **래티스(Lattice)** 구조로 관리된다. 래티스란 "한 방향으로만 진행할 수 있는 상태 기계"라고 이해하면 쉽다. 처음에는 아무것도 관찰하지 않은 상태에서 시작하여, 관찰된 타입에 따라 점점 더 일반적인 상태로 진행한다.
...
왜 이렇게 설계했을까? 만약 피드백 상태가 왔다갔다 할 수 있다면, V8은 끝없는 최적화-취소 루프에 빠질 수 있다. 정수로 최적화 → 부동소수점 등장으로 취소 → 다시 정수만 와서 정수로 최적화 → 또 부동소수점... 이런 상황이 반복되면 자바스크립트를 실행하는 대신 최적화와 취소만 반복하게 된다. 단방향 래티스 구조는 이런 문제를 원천 차단한다[^2].

### 1-1-1-6. Node.js에서 V8의 역할
...
V8은 파싱 비용을 줄이기 위해 **지연 파싱(Lazy Parsing)** 기법을 사용한다. 모든 함수를 처음부터 완전히 파싱하지 않고, 함수 선언만 기록해두었다가 실제로 호출될 때 완전한 파싱을 수행한다. 예를 들어 다음과 같은 코드가 있다고 하자.
...
V8 공식 블로그에 따르면, Ignition이 생성하는 바이트코드는 기존 baseline 컴파일러가 생성하던 기계어 코드 대비 25~50% 수준의 크기를 가진다[^3]. 이러한 크기 감소는 특히 메모리가 제한된 모바일 환경에서 중요하다. V8 팀은 Ignition 도입으로 크롬 탭당 약 5%의 메모리 사용량이 감소했다고 밝혔다.

다음은 위의 `add` 함수가 어떤 형태의 바이트코드로 변환되는지 보여주는 예시다.

```bash
# 바이트코드 확인을 위한 Node.js 실행
...
위 바이트코드를 해석하면 다음과 같다. `Ldar a1`은 두 번째 인자(`b`)를 누산기(accumulator) 레지스터에 로드한다. `Add a0, [0]`은 첫 번째 인자(`a`)와 누산기의 값을 더해 결과를 누산기에 저장한다. `[0]`은 피드백 벡터의 슬롯 인덱스로, 이 연산에서 관찰된 타입 정보가 저장되는 위치다. `Return`은 누산기의 값을 반환한다.

Ignition은 **레지스터 머신** 아키텍처를 사용한다. 스택 기반 가상 머신(예: JVM)과 달리, 레지스터 머신은 연산 결과를 레지스터에 저장하고 참조한다. V8은 특히 **누산기(accumulator)** 라는 특수 레지스터를 활용하여 많은 바이트코드가 암시적으로 이 레지스터를 입출력으로 사용하게 함으로써 바이트코드 크기를 더욱 줄였다.

Ignition의 실행 방식을 더 자세히 살펴보자. Ignition은 각 바이트코드에 대응하는 **핸들러(handler)** 를 가지고 있다. 이 핸들러들은 TurboFan의 중간 표현(IR)을 사용해 작성되어, V8이 지원하는 모든 CPU 아키텍처(x64, ARM64, ARM32 등)에 대해 최적화된 기계어 코드로 컴파일된다[^4]. 즉, 인터프리터의 핵심 루프 자체가 고도로 최적화되어 있다.

```mermaid
flowchart LR
    subgraph Ignition["Ignition 실행 루프"]
        Fetch["바이트코드 페치"] --> Decode["디코드"]
        Decode --> Handler["핸들러 실행"]
        Handler --> Dispatch["다음 핸들러로<br/>디스패치"]
        Dispatch --> Fetch
    end

    Handler --> |"타입 정보 기록"| FV["피드백 벡터"]
```

함수가 호출되면 **InterpreterEntryTrampoline** 이라는 스텁이 실행되어 적절한 스택 프레임을 설정한다. 그 후 첫 번째 바이트코드의 핸들러로 점프하여 실행을 시작한다. 각 핸들러는 자신의 작업을 수행한 뒤, **디스패치 테이블** 을 통해 다음 바이트코드의 핸들러로 직접 점프(tail call)한다. 이 방식을 **간접 스레디드 인터프리터(indirectly threaded interpreter)** 라고 부른다.

그러나 아무리 최적화해도 인터프리터에는 본질적인 오버헤드가 존재한다. 바이트코드를 디코딩하는 비용, 다음 핸들러로 디스패치하는 비용, 그리고 CPU가 다음에 실행될 명령어를 예측하기 어려워 발생하는 분기 예측 실패 비용 등이 그것이다. 이러한 한계를 극복하기 위해 V8은 최적화 컴파일러를 함께 사용한다.

### 1-1-2-3. TurboFan: 최적화 컴파일러
...
TurboFan은 **Sea of Nodes** 라는 중간 표현(IR)을 사용한다[^5]. 전통적인 컴파일러가 제어 흐름 그래프(CFG)와 데이터 흐름을 별도로 관리하는 것과 달리, Sea of Nodes는 이 둘을 통합된 그래프로 표현한다. 이 설계 덕분에 명령어 재정렬이나 중복 계산 제거 같은 최적화가 더 효과적으로 이루어진다.

```mermaid
flowchart TB
    subgraph SeaOfNodes["Sea of Nodes IR"]
        Start["Start"] --> Param1["Parameter 0"]
        Start --> Param2["Parameter 1"]

        Param1 --> Add["NumberAdd"]
        Param2 --> Add
...
TurboFan이 적용하는 주요 최적화 기법들을 살펴보자[^5].
...
인라이닝의 진정한 위력은 단순히 함수 호출 오버헤드를 제거하는 데 있지 않다. **인라이닝 후에 다른 최적화들이 연쇄적으로 적용될 수 있다는 점**이 핵심이다. 다음 예시를 통해 이 과정을 자세히 살펴보자[^6].
...
// TurboFan 최적화 후: 객체 할당 없이 직접 계산
// dx = x2 - x1, dy = y2 - y1, return sqrt(dx*dx + dy*dy)
```

이스케이프 분석이 특히 효과적인 또 다른 사례는 **Rest Parameters**다[^6]. ES6에서 도입된 Rest Parameters(`...args`)는 가변 인자를 배열로 수집하는 편리한 문법이다. 하지만 내부적으로는 매 호출마다 새로운 배열 객체가 힙에 할당되어야 한다.
...
**루프 최적화** 는 반복문의 성능을 개선한다. **루프 불변 코드 이동(Loop Invariant Code Motion)** 은 루프 내에서 결과가 변하지 않는 계산을 루프 밖으로 옮긴다. **루프 언롤링(Loop Unrolling)** 은 루프 본문을 여러 번 복제하여 반복 횟수를 줄이고, 루프 제어 오버헤드를 감소시킨다.

**범위 분석(Range Analysis)** 은 변수가 가질 수 있는 값의 범위를 추적하여 불필요한 검사를 제거한다. 예를 들어, 배열 인덱스가 항상 유효한 범위 내에 있다고 증명되면 경계 검사를 생략할 수 있다.

### 1-1-2-4. Maglev: 중간 계층 컴파일러
...
Maglev의 설계는 TurboFan과 다른 철학을 따른다. TurboFan이 Sea of Nodes라는 복잡한 IR을 사용하는 반면, Maglev는 전통적인 **SSA(Static Single Assignment) 기반의 제어 흐름 그래프** 를 사용한다[^7]. 이 선택은 의도적인 것으로, 컴파일 속도를 높이기 위해 일부 최적화 기회를 의도적으로 포기한 것이다.

```mermaid
flowchart LR
    subgraph 컴파일속도["컴파일 속도"]
        direction TB
        I["Ignition<br/>(즉시)"]
        M["Maglev<br/>(빠름)"]
        T["TurboFan<br/>(느림)"]
    ...
    style M fill:#f9d77e
    style M2 fill:#f9d77e
```

Maglev의 컴파일 과정은 다음과 같이 진행된다. 먼저 **사전 패스(prepass)** 에서 바이트코드를 분석하여 분기와 루프 변수를 파악한다. 그다음 **그래프 빌딩** 단계에서 바이트코드를 SSA 노드로 변환한다. 여러 제어 경로에서 오는 값을 병합할 때는 **Phi 노드** 를 사용한다. 마지막으로 **레지스터 할당기** 가 단일 전방 패스로 레지스터를 할당한다[^7].

Maglev의 또 다른 특징은 **추측적 최적화를 지원** 한다는 것이다. Maglev도 피드백 벡터의 타입 정보를 활용하여 최적화된 코드를 생성하고, 타입 가정이 깨지면 디옵티마이제이션을 수행한다. 이를 위해 디옵티마이제이션이 필요한 노드에는 **인터프리터 프레임 상태** 가 첨부되어, 안전하게 Ignition으로 돌아갈 수 있다.
...
디옵티마이제이션에도 종류가 있다. 각각이 언제 발생하는지 구체적인 상황과 함께 살펴보자[^2].
...
Node.js에서 디옵티마이제이션 발생 여부와 이유를 확인하려면 다음 플래그들을 사용할 수 있다.
...
# 더 자세한 정보 (이유 포함)
node --trace-deopt --trace-deopt-verbose script.js

# 최적화 과정 전체 추적
node --trace-opt --trace-deopt script.js
```

출력 예시를 보면 어떤 함수가 왜 디옵티마이제이션되었는지 알 수 있다.

```
[deoptimizing (DEOPT eager): begin ... calculate]
    ;;; deoptimize at <script.js:2:10>, not a Smi
```

위 메시지는 `calculate` 함수가 `script.js`의 2번째 줄 10번째 열에서 "not a Smi"(Small Integer가 아님) 이유로 디옵티마이제이션되었음을 보여준다.
...
인라인 캐싱(Inline Caching, IC)은 바로 이 문제를 해결하기 위한 기법이다. V8 엔지니어들이 "자바스크립트를 빠르게 만드는 핵심 요소"라고 설명할 정도로 중요한 최적화 기법이다[^8]. 이 기법의 핵심 아이디어도 앞서 본 추측적 최적화와 같다. **"한 번 찾은 정보를 기억해두고 재사용하자."**

### 1-1-3-1. 왜 속성 접근이 느릴 수 있는가
...
### 1-1-3-2. 히든 클래스: 객체의 "설계도"

인라인 캐싱이 어떻게 동작하는지 이해하려면 먼저 V8이 객체를 어떻게 바라보는지 알아야 한다.

자바스크립트에서 객체는 매우 자유롭다. 언제든 속성을 추가하거나 삭제할 수 있다. 하지만 실제 코드를 보면, 같은 "종류"의 객체가 반복적으로 생성되는 경우가 많다. V8은 이 점을 활용한다. **같은 구조를 가진 객체들은 "설계도"를 공유하게 만드는 것이다.** 이 설계도를 V8 내부에서는 **Map** 이라고 부르고, 다른 문헌에서는 **히든 클래스(Hidden Class)** 또는 **Shape** 라고 부른다.
...
객체에 속성을 추가하면 히든 클래스가 변경된다. 이 변경은 **트랜지션 체인(Transition Chain)** 으로 연결된다.
...
### 1-1-3-4. IC의 상태 변화: 단형에서 거대형까지

인라인 캐시는 코드의 각 속성 접근 위치마다 생성된다. 그리고 실행 중에 어떤 종류의 객체를 만나느냐에 따라 상태가 변화한다. 이 상태는 성능에 직접적인 영향을 미치므로 이해해둘 필요가 있다[^10].
...
### 1-1-4-3. 배열 요소 타입의 일관성

V8은 배열 요소의 타입에 따라 다른 내부 표현을 사용한다[^9]. 정수만 담긴 배열(PACKED_SMI_ELEMENTS), 실수를 포함한 배열(PACKED_DOUBLE_ELEMENTS), 객체를 포함한 배열(PACKED_ELEMENTS) 등이 구분된다.
...
또한 배열에 "구멍(hole)"이 생기지 않도록 주의해야 한다. 구멍이 있는 배열(HOLEY)은 요소 접근 시 프로토타입 체인 확인이 필요해 느려진다. 구멍이 있는 배열에서 `arr[i]`를 읽을 때, V8은 해당 인덱스가 진짜 비어있는 것인지, 아니면 `Array.prototype`에 해당 인덱스의 값이 정의되어 있는지 확인해야 한다.

```javascript
// 안 좋은 예: 구멍이 있는 배열
const holey = [1, 2, , 4, 5]; // 인덱스 2가 비어있음
const sparse = [];
sparse[1000] = 1; // 희소 배열
...
// 배열 생성 시 주의
const arr = new Array(100); // HOLEY 배열 생성!
...
### 1-1-4-4. try-catch와 최적화

과거에는 `try-catch` 블록 내부의 코드가 최적화되지 않는 문제가 있었다. 현재 V8에서는 이 제한이 크게 완화되었지만, 에러가 빈번하게 발생하는 코드 경로는 여전히 최적화에 불리하다.
...
### 1-1-4-5. 동적 키 관리에는 Object 대신 Map 사용하기
...
[^3]: Firing up the Ignition interpreter - <https://v8.dev/blog/ignition-interpreter>
[^4]: Understanding V8's Bytecode - <https://medium.com/nicotec/understanding-v8s-bytecode-317d46c94775>
[^5]: Digging into the TurboFan JIT - <https://v8.dev/blog/turbofan-jit>
[^6]: V8 release v7.1 - Escape Analysis improvements - <https://v8.dev/blog/v8-release-71>
...
[^9]: Fast properties in V8 - <https://v8.dev/blog/fast-properties>
[^10]: What's up with monomorphism? - <https://mrale.ph/blog/2015/01/11/whats-up-with-monomorphism.html>
[^11]: V8 source code: flag-definitions.h - <https://github.com/v8/v8/blob/main/src/flags/flag-definitions.h>
[^12]: AST Explorer - <https://astexplorer.net/>
[^13]: Blazingly fast parsing, part 2: lazy parsing - <https://v8.dev/blog/preparser>
